...
在对比框架之前，先看看实践中常见的选型误区，你可能会有共鸣。

### 陷阱一：被 "最流行" 误导

一个典型的错误是只看 GitHub star 数量（当时 LangChain 有 80k+）就盲目选择。
...
LangChain 更新太快，之前写的代码过了一个月就跑不通了。
...
以下是一组基准测试结果，同一个 ReAct Agent 用不同框架实现，测试 100 次工具调用。
