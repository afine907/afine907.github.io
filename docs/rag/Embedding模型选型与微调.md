...
> 在实际 RAG 项目中，一个常见的场景是：用户问"合同违约金上限是多少"，系统检索回来的文档却是关于"劳动法试用期规定"的——语义完全不相关。排查后往往发现，不是检索逻辑有问题，而是 Embedding 模型对特定领域（如法律术语）的语义理解不够好。这类问题揭示了一个关键事实：**Embedding 模型选型是 RAG 系统中最容易被忽视、但影响最大的环节**。
...
在一个中文技术文档数据集（10 万条）上进行评测，测试任务是语义检索的 Recall@5：
...
这是一个容易踩坑的问题。很多人以为 OpenAI 的模型什么语言都好用，但实际情况是：
...
**问题**：之前用 `bge-small-zh-v1.5`（维度 512）构建了向量索引，后来想换 `bge-large-zh`（维度 1024），直接切换模型后 Chroma 报错 `Dimension mismatch`。
...
**问题**：使用 `text-embedding-3-small` 的早期版本构建了向量索引，某天 OpenAI 更新了模型版本（没有大版本号变化），新生成的 Embedding 和旧的不兼容，检索质量突然暴跌。
