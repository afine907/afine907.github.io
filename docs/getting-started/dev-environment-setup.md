...
换了终端窗口后直接 `pip install`，结果装到了全局 Python。项目跑不起来，因为虚拟环境里啥也没有。**每次开新终端第一件事：激活虚拟环境。**

**坑 2：API Key 提交到了 GitHub**

第一次做项目，把 API Key 直接写在代码里提交了。10 分钟后收到 OpenAI 的警告邮件——有人扫到了我的 Key 开始滥用。**用 `.env` + `.gitignore` 保护好你的 Key。**
