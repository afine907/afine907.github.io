...
一个典型的场景是：Agent 上线后单次对话消耗了 50 万 Token——账单惊人。原因是没有做任何 Token 管理，把所有历史消息一股脑全塞进上下文。加上滑动窗口压缩后，成本降了 80%。

**陷阱 2：Temperature 设太高导致 Agent "发疯"**

如果把 Temperature 设到 0.8，想让 Agent 回答更"生动"，结果 Agent 在工具选择上会"自由发挥"——有时候选择搜索、有时候选择读文件、有时候什么都不选直接编答案。**Agent 的决策部分 Temperature 一定要低（0-0.3）。**

**陷阱 3：以为"上下文窗口大"就可以随便塞**

常见的错误是换了 200K 模型后，把整本 API 文档都塞进 System Prompt。结果 Agent 开始忽略一些关键指令——不是没看到，是"注意力"被稀释了。**大窗口不是免死金牌，信息密度比信息总量更重要。**
