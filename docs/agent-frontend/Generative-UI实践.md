...
在实际项目中，一个常见的需求是：用户输入"帮我查一下上周的销售数据"，AI 不仅返回数据，还要决定用什么图表展示——柱状图、折线图还是表格。

最初的做法是前端写死一个 `ChartRenderer`，LLM 返回 `{ type: "bar", data: [...] }`，前端 `switch` 渲染。结果呢？每新增一种可视化方式，都要改前端代码、改后端 schema、改 prompt。多人协作时容易互相踩脚，维护成本急剧上升。
