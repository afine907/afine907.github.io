...
在 Agent 前端开发中，一个常见的问题是：后端返回的结果是流式的，一个字一个字蹦出来，用户看到的只是一个光标在那里闪啊闪，完全不知道 Agent 在干嘛。它在思考？在调用工具？还是卡住了？

这类体验问题在实践中非常普遍——后端的 Agent 能力做得不错，但前端的体验却很糟糕。为此，一个可行的方案是构建 React 组件库（如 `agent-sse-flow`），专门用来可视化 Agent 的 SSE 流式响应。

本文将介绍这类组件库的设计思路、实现细节、性能优化和常见的坑，帮助你在 Agent 前端开发中解决 SSE 流式渲染的体验问题。
...
整个可视化可以拆成几个层级的组件，每个组件只管自己那部分逻辑。
