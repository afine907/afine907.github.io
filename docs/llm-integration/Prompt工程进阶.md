...
> 很多人一开始觉得 Prompt 不就是写几句话嘛，能有多难？但一个典型的"踩坑"场景是：Agent 在生产环境连续返回错误格式的数据，排查许久才发现是 System Prompt 里少了一句"请用 JSON 格式返回"。这类教训说明：Prompt 工程不是"写几句话"，而是一门需要系统化方法论的工程学科。
